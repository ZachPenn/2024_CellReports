...
Only animals from NT and T: 1 mA group are considered here, for simplicity.
...
groups$Exclude[groups$Group=='T: Low'] = 1

#refactor groups
groups$Group = recode_factor(
  groups$Group,
  'NT' = 'NT', 'T: Low' = 'T: low', 'T: High' = 'T', 'T: 2.0 mA' = 'T: 2.0 mA'
  )
...
data_epm = right_join(x=groups,y=data_epm,by=c("Animal","AnimalID"))
...
sefltest.min = data_vf[data_vf$phase=='sefltest' & data_vf$subphase!='avg',]
...
#p.margin
p.margin1 = unit(c(2, 3.6, 2.2, 3.6), "cm")
p.margin2 = unit(c(1.25, 2.5, 2.15, 2.5), "cm") # for sex comparisons
p.margin3 = unit(c(2, 3., 2.45, 3.), "cm") #for trauma
p.margin4 = unit(c(1.25, 2, 2.45, 2), "cm") #for trauma by sex
...
if (p.save == TRUE) {saveplot(filename = paste(f_pfx, "EPMpost_Closed.tiff", sep = '_'))}
...
### Correlations for Trauma 
```{r, fig.width=4.5, fig.height=4.5, }

subdata = freezing.df[freezing.df$Group=='T',]
...
for (i in 1:length(cor_list)){
  ...
  cor_plots[[n]] = ggplot(
    data = subdata,
    aes_string(x = pair[1], y = pair[2], fill="Sex")
  ) +
