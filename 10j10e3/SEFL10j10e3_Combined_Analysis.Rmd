...
groups$Group = recode_factor(groups$Group, 'NT - Veh'='NT: veh', 'T - Veh'='T: veh')
...
p.scale.colour = c("gray28","white","red3", "royalblue3")
p.scale.fill = c("gray28","white","red3", "royalblue3")
...
#Create base layer from group level data
P.ctrls.Trauma.Freezing = ggplot(
    data = summarise_at(
    .tbl = group_by(
      .data = trauma.freezing[trauma.freezing$drug == 'Veh',],
      subphase,component,Group,region),
    .funs = list(mean=~mean(.), sem=~sd(.)/sqrt(n())),
    .vars = c('motion','freezing')
  ),
  aes(x=component,
      ...
      fill=Group,
      shape=region,
      group=interaction(subphase,Group,region)))
...
#Do not include NT: veh vs. T: veh in posthoc comparisons
...
ctrl.trauma.anova = ezANOVA(
  data = trauma.freezing[trauma.freezing$drug=='Veh',],
  dv = freezing,
  wid = Animal,
  between = .(Group, region),
  within = component,
  white.adjust = TRUE,
  type = 3,
  return_aov = TRUE
